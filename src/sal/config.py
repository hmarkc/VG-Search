...
class Config:
    ...
    level: int = None # level of the math problem, should be [1,5]
    ...
    # Granularity options
    g: int = 1  # granularity of the search space
    ...
    def __post_init__(self):
        ...
        if not self.push_to_hub and self.approach == "beam_search":
            self.revision = f"{self.dataset_name.replace('/', '_')}--beam_search--T-{self.temperature}--top_p-{self.top_p}--n-{self.n}--m-{self.beam_width}--iters-{self.num_iterations}--look-{self.lookahead}--seed-{self.seed}--agg_strategy--{self.agg_strategy}--granularity-{self.g}"
        elif not self.push_to_hub and self.approach == "dvts":
            self.revision = f"{self.dataset_name.replace('/', '_')}--dvts--T-{self.temperature}--top_p-{self.top_p}--n-{self.n}--seed-{self.seed}--agg_strategy-{self.agg_strategy}"
