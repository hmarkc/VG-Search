...
def get_dataset(config: Config) -> Dataset:
    ...
    # select the dataset with specific level
    if config.level is not None:
        dataset = dataset.filter(lambda x: x["level"] == config.level)
...
def save_dataset(dataset, config):
    if config.push_to_hub:
        ...
        dataset.to_json(
            ...
            f"{config.output_dir}/{config.revision}.jsonl", lines=True
        )
        logger.info(
            # f"Saved completions to {config.output_dir}/{config.approach}_completions.jsonl"
            f"Saved completions to {config.output_dir}/{config.revision}.jsonl"
        )
